...
// ✅ FIX: Import Resume model to fetch user's latest resume
...
exports.applyForJob = async (req, res) => {
  try {
    ...
    // ✅ FIX: Look up user's latest resume from Resume collection
    // Old code: if (!req.user.resume) — this field doesn't exist on User model
    ...
    if (!userResume) {
      return res.status(400).json({
        success: false,
        message: 'Please create a resume before applying for jobs',
      });
    }
    ...
    // ✅ FIX: Build resumeUrl from resume data instead of req.user.resume
    // Use personalInfo name as identifier or store resume ID as string
    const resumeUrl = `/resumes/${userResume._id}`;

    const application = await Application.create({
      jobId,
      jobSeekerId: req.user._id,
      resumeUrl,                    // ✅ Now has a valid value
      resumeId: userResume._id,     // ✅ Also store reference for easy lookup
      coverLetter,
      statusHistory: [{
        status: 'Applied',
        changedAt: new Date(),
...
exports.getJobApplications = async (req, res) => {
  try {
    ...
    const applications = await Application.find({ jobId: req.params.id })
      .populate('jobSeekerId', 'name email phone skills experience profilePicture')
      .sort({ createdAt: -1 });
